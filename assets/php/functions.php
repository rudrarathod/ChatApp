...
//for checking duplicate enrollment
function isUsernameRegistered($enrollment)
{
    global $db;
    $query = "SELECT count(*) as row FROM users WHERE enrollment='$enrollment'";
    $run = mysqli_query($db, $query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}

//for checking duplicate enrollment by other
function isUsernameRegisteredByOther($enrollment)
{
    global $db;
    $user_id = $_SESSION['userdata']['id'];
    $query = "SELECT count(*) as row FROM users WHERE enrollment='$enrollment' && id!=$user_id";
    $run = mysqli_query($db, $query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}
...
function validateSignupForm($form_data)
{
    ...
    if (!$form_data['scheme']) {
        $response['msg'] = "scheme is not selected";
        $response['status'] = false;
        $response['field'] = 'scheme';
    }

    if (!$form_data['sem']) {
        $response['msg'] = "sem is not selected";
        $response['status'] = false;
        $response['field'] = 'sem';
    }

    if (!$form_data['branch']) {
        $response['msg'] = "branch is not selected";
        $response['status'] = false;
        $response['field'] = 'branch';
    }

    if (!$form_data['enrollment']) {
        $response['msg'] = "enrollment is not given";
        $response['status'] = false;
        $response['field'] = 'enrollment';
    }
    ...
    if (isUsernameRegistered($form_data['enrollment'])) {
        $response['msg'] = "enrollment is already registered";
        $response['status'] = false;
        $response['field'] = 'enrollment';
    }
...
function validateLoginForm($form_data)
{
    ...
    if (!$form_data['username_email']) {
        $response['msg'] = "enrollment/email is not given";
        $response['status'] = false;
...
function checkUser($login_data)
{
    ...
    $query = "SELECT * FROM users WHERE (email='$username_email' || enrollment='$username_email') && password='$password'";
    $run = mysqli_query($db, $query);
...
function getfilterPosts($allPosts, $formData)
{
    $filteredList = array();

    foreach ($allPosts as $post) {
        // Check if the post matches the form input criteria
        if (
            (empty($formData['branch']) || $formData['branch'] == $post['branch']) &&
            (empty($formData['sem']) || $formData['sem'] == $post['sem']) &&
            (empty($formData['scheme']) || $formData['scheme'] == $post['scheme']) &&
            (empty($formData['subject']) || $formData['subject'] == $post['subject'])
        ) {
            $filteredList[] = $post;
        }
    }
...
{
    global $db;
    $query = "SELECT syllabus.id, syllabus.code, syllabus.branch, syllabus.sem, syllabus.scheme, syllabus.syllabus_data, users.id as uid, users.first_name, users.last_name, users.enrollment, users.profile_pic FROM syllabus JOIN users ON users.id = syllabus.user_id ORDER BY syllabus.id DESC";
...
{
    global $db;
    $query = "SELECT * FROM users WHERE enrollment LIKE '%" . $keyword . "%' || (first_name LIKE '%" . $keyword . "%' || last_name LIKE '%" . $keyword . "%') LIMIT 5";
    $run = mysqli_query($db, $query);
    return mysqli_fetch_all($run, true);
}
...
//for getting userdata by enrollment
function getUserByUsername($enrollment)
{
    global $db;
    $query = "SELECT * FROM users WHERE enrollment='$enrollment'";
    $run = mysqli_query($db, $query);
    return mysqli_fetch_assoc($run);
}
...
function getPost()
{
    global $db;
    $current_user = $_SESSION['userdata']['id'];
    $query = "SELECT users.id as uid, posts.id, posts.user_id, posts.post_img, posts.post_text, posts.created_at, posts.branch, posts.sem, posts.scheme, users.first_name, users.last_name, users.enrollment, users.profile_pic, users.branch, users.sem, users.scheme FROM posts JOIN users ON users.id = posts.user_id ORDER BY posts.id DESC";
...
function filterPosts()
{
    ...
    // Initialize an empty array to store filtered posts
    $filter_list = array();
    
    // Iterate through each post in the list
    foreach ($list as $post) {
        // Check if the user is following the post author or if post details match the user's branch, sem, and scheme
        if ($post['user_id']!=$_SESSION['userdata']['id']) {
            if (checkFollowStatus($post['user_id']) || 
            ($post['branch'] == $_SESSION['userdata']['branch'] && 
             $post['sem'] == $_SESSION['userdata']['sem'] && 
             $post['scheme'] == $_SESSION['userdata']['scheme'])) {
            // If the conditions are met, add the post to the filtered list
            $filter_list[] = $post;
        }        }
    }
...
//     $enrollment = mysqli_real_escape_string($db, $data['enrollment']);
//     $branch = mysqli_real_escape_string($db, $data['branch']);
//     $sem = mysqli_real_escape_string($db, $data['sem']);
//     $scheme = mysqli_real_escape_string($db, $data['scheme']);
//     $password = mysqli_real_escape_string($db, $data['password']);
//     $password = md5($password);

//     $query = "INSERT INTO users(first_name,last_name,gender,email,enrollment,branch,sem,scheme,password) ";
//     $query .= "VALUES ('$first_name','$last_name',$gender,'$email','$enrollment','$branch','$sem','$scheme','$password')";
//     return mysqli_query($db, $query);
// }
function createUser($data)
{
    ...
    $query = "INSERT INTO users(first_name,last_name,gender,email,enrollment,branch,sem,scheme,password,ac_status) ";
    $query .= "VALUES ('$first_name','$last_name','$gender','$email','$enrollment','$branch','$sem','$scheme','$password','1')";
    return mysqli_query($db, $query);
}
...
function validateUpdateForm($form_data, $image_data)
{
    ...
    if (isUsernameRegisteredByOther($form_data['enrollment'])) {
        $response['msg'] = $form_data['enrollment'] . " is already registered";
        $response['status'] = false;
        $response['field'] = 'enrollment';
    }
...
function updateProfile($data, $imagedata)
{
    ...
    $query = "UPDATE users SET first_name = '$first_name', last_name='$last_name',password='$password' $profile_pic WHERE id=" . $_SESSION['userdata']['id'];
    return mysqli_query($db, $query);
}
...
function validatePostImage($image_data)
{
    ...
    if ($image_data['name']) {
        ...
        if (!in_array(strtolower($type), $allowed_types)) {
            $response['msg'] = "Only JPG, JPEG, PNG, PDF files are allowed";
            $response['status'] = false;
            $response['field'] = 'post_file';
        }
...
function createPost($text, $image)
{
    ...
    $branch = mysqli_real_escape_string($db, $text['branch']);
    $sem = mysqli_real_escape_string($db, $text['sem']);
    $scheme = mysqli_real_escape_string($db, $text['scheme']);

    $subject = mysqli_real_escape_string($db, $text['SUBJECT']);


    $query = "INSERT INTO posts(user_id,post_text,post_img,branch,sem,scheme,subject)";
    $query .= "VALUES ($user_id,'$post_text','$image_name','$branch','$sem','$scheme','$subject')";
    return mysqli_query($db, $query);
}
